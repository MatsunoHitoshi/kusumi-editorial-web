...
export function PublishPanel() {
  ...
  return (
    <section className="rounded border border-zinc-200 p-4">
      <h2 className="text-lg font-medium">サイトの公開反映</h2>
      <p className="mt-2 text-sm text-zinc-600">
        公開中（<code className="rounded bg-zinc-100 px-1">published</code>）のコンテンツに新しい{" "}
        <code className="rounded bg-zinc-100 px-1">publishVersion</code> を付与し、GitHub Actions
        で静的サイトをビルドして GitHub Pages に反映します。誤操作防止のため最終確認があります。
      </p>
      {me?.role === "admin" ? (
      ...
      >
        ...
        >
          ...
          <p className="mt-2 text-sm text-zinc-600">
            次の一覧がそのまま新しい publishVersion でタグ付けされ、サイトビルドの対象になります。公開すべきでない項目が{" "}
            ...
            <>
              ...
              <div className="mt-4 flex items-start gap-2">
                ...
                <label htmlFor={confirmCheckboxId} className="text-sm text-zinc-800">
                  上記の公開対象で問題ないことを確認し、デプロイをキューしてよい
                </label>
              </div>
              <div className="mt-3">
